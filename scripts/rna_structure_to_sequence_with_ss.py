#!/usr/bin/env python3
"""
Script: rna_structure_to_sequence_with_ss.py
Description: RNA structure-to-sequence design with secondary structure constraints

Original Use Case: examples/use_case_1_structure_to_sequence_with_ss.py
...
Usage:
    python scripts/rna_structure_to_sequence_with_ss.py --pdb FILE --ss FILE --output FILE
...
# Local utilities
try:
    from lib.io import save_fasta, load_numpy, save_json
    ...
    save_fasta, load_numpy, save_json = rna_io.save_fasta, rna_io.load_numpy, rna_io.save_json
...
def run_structure_to_sequence_with_ss(
    ...
    **kwargs
    ...
    if not ss_file.exists():
        raise FileNotFoundError(f"Secondary structure file not found: {ss_file}")
    ...
    try:
        ...
        ss_contact_map = load_secondary_structure(ss_file)
        ...
        if output_file:
            ...
            # Save FASTA
            header = f"RhoDesign_prediction_with_SS_recovery_{recovery_rate:.3f}"
...
def main():
    ...
    parser.add_argument('--ss', '-s', required=True,
                      help='Path to .npy file containing secondary structure contact map')
